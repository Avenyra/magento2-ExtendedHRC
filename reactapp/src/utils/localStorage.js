...
const LocalStorage = {
  ...
  getBillingSameAsShippingInfo() {
    ...
    const value = _get(LocalStorage.getHyvaCheckoutStorage(), source.value);
    return value !== false;
  },
  ...
  getDeliveryNote() {
    ...
    return window.localStorage.getItem('hyva_checkout_delivery_note') || '';
  },

  saveDeliveryNote(note) {
    if (!LocalStorage.isBrowser()) {
      return;
    }
    window.localStorage.setItem('hyva_checkout_delivery_note', note);
  },
