...
function modifyCartItemsData(cartItems) {
  return cartItems.reduce((accumulator, item) => {
    ...
    let productSmallImgUrl = _get(product, 'small_image.url') || '';
    if (
      productSmallImgUrl &&
      !productSmallImgUrl.startsWith('http://') &&
      !productSmallImgUrl.startsWith('https://')
    ) {
      const baseUrl = (config.baseUrl || '').replace(/\/$/, '');
      const relativeUrl = productSmallImgUrl.startsWith('/')
        ? productSmallImgUrl
        : `/${productSmallImgUrl}`;
      productSmallImgUrl = `${baseUrl}${relativeUrl}`;
    }
