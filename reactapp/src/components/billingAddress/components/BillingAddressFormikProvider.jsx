...
function BillingAddressFormikProvider({ children, formikData }) {
  ...
  const { customerAddressList, defaultBillingAddress } =
    useBillingAddressAppContext();
  ...
  // When checkbox is unchecked and user has addresses, show address list (view mode)
  // and auto-select the default billing address if none is selected yet
  useEffect(() => {
    if (!isSame && customerHasAddress(customerAddressList)) {
      setFormToViewMode();
      if (!isValidCustomerAddressId(selectedAddress) && defaultBillingAddress) {
        setSelectedAddress(_toString(defaultBillingAddress));
        setCustomerAddressSelected(true);
      }
    }
  }, [isSame]); // eslint-disable-line react-hooks/exhaustive-deps
