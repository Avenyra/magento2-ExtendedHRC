...
function ShippingAddressView() {
  ...
  const handleEditAddress = (addressId) => {
    ...
    const addressToBackup = customerAddress || cartShippingAddress;
  ...
  const handleSelectAddress = async (addressId) => {
    ...
    // Save shipping address and trigger methods fetch
    await submitHandler(addressId);
  };

  // --- Guest View ---
  if (!isLoggedIn) {
    const isCartValid = isCartAddressValid(cartShippingAddress);
    ...
    return (
      <div className="border border-gray-200 rounded-lg p-5 bg-white shadow-sm flex items-center justify-between transition-all duration-300 hover:shadow-md mt-4">
    ...
    <div className="mt-4 space-y-4">
      {addresses.length === 0 ? (
        <div className="text-center py-6 border-2 border-dashed border-gray-200 rounded-lg bg-gray-50 text-gray-500">
          {__('No saved addresses found. Please add a new shipping address.')}
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {displayAddresses.map((addr) => {
            ...
            const isDefault = addr.isDefaultShipping;
