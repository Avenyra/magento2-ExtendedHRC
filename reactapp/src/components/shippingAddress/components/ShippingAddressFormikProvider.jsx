...
import { _toString, _isObjEmpty } from '../../../utils';
...
function ShippingAddressFormikProvider({ children, formikData }) {
  ...
  const { customerAddressList, defaultShippingAddress, isLoggedIn } =
    useShippingAddressAppContext();
  ...
  // Auto-select default shipping address when no cached ID or non-customer ID is cached, and customer is logged in
  useEffect(() => {
    if (
      !isLoggedIn ||
    ...
    if (!isValidCustomerAddressId(selectedAddress)) {
      const defaultId = _toString(defaultShippingAddress);
      setSelectedAddress(defaultId);
      setCustomerAddressSelected(true);
      LocalStorage.saveCustomerShippingAddressId(defaultId);
      setFormToViewMode();
    }
  }, [
    isLoggedIn,
